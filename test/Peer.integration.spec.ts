...
import { untilTrue, whileTrue } from './test-utils'
...
describe('Peer Integration Test', function () {
  ...
  it('Sends and receives data', async () => {
    ...
    const received = await sendMessage(peer2, peer1, 'room', {
      hello: 'world'
    })
  ...
  xit('suspends relay when receiving duplicate or expired', async () => {
    ...
    await untilTrue(
      () => (peer2 as any).isRelayFromConnectionSuspended(peer1.peerIdOrFail(), peer3.peerIdOrFail()),
      'Awaiting for peer2 to have asked peer1 to suspend relays for peer3'
    )
    await untilTrue(
      () => (peer2 as any).isRelayToConnectionSuspended(peer2.peerIdOrFail(), peer3.peerIdOrFail()),
    ...
    await untilTrue(
      () => (peer2 as any).isRelayFromConnectionSuspended(peer4.peerIdOrFail(), peer3.peerIdOrFail()),
  ...
  function assignId(socket: SocketMock, id: string = 'assigned') {
    socket.onmessage({
      data: JSON.stringify({
        type: ServerMessageType.AssignedId,
        payload: { id }
      })
    })
  }
  ...
  function openConnection(socket: SocketMock) {
    socket.onmessage({
      data: JSON.stringify({ type: ServerMessageType.Open })
    })
    socket.onmessage({
      data: JSON.stringify({ type: ServerMessageType.ValidationOk })
    })
  }
